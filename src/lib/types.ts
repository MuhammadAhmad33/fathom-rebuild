...
export type ActionItem = { id: string; text: string; owner: string; time: number; done: boolean };
export type Highlight = { id: string; title: string; type: string; start: number; end: number; note: string };
export type Meeting = { id: string; title: string; company: string; date: string; duration: number; kind: 'internal' | 'external'; color: string; people: Person[]; transcript: Segment[]; summary: Summary; actions: ActionItem[]; highlights: Highlight[]; media: string; description: string };
